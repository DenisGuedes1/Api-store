...
import { TreturnCreatedProducts, TupdateProducts} from "../../interface/interfaceProducts";
import { AppError } from "../../error/handleError";
import { Repository } from "typeorm";
import {updateProductsSchema} from "../../schema/schema.products"
export const updateProductsService = async(productsData: Partial<TreturnCreatedProducts>, idProducts:number) =>{

    const productsRepository: Repository<Products> = AppDataSource.getRepository(Products);
     
    const findProducts = await productsRepository.findOne({where:{id: idProducts}});
